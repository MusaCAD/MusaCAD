...
MainWindow::MainWindow(QWidget* parent) : QMainWindow(parent) {
    ...
    connect(title_timer_, &QTimer::timeout, this, [this] {
        ...
        refresh_property_combos(); // the ribbon's colour / linetype / lineweight controls
...
bool MainWindow::ribbon_shot(int kind, const std::string& out_png) {
    ...
    // MUSACAD_RIBBON_TAB=<index>: capture another tab (Insert, Annotate, ...) at this width.
    if (bool tab_ok = false; qEnvironmentVariableIsSet("MUSACAD_RIBBON_TAB")) {
        const int tab = qEnvironmentVariable("MUSACAD_RIBBON_TAB").toInt(&tab_ok);
        if (auto* tb = ribbon_->findChild<QTabBar*>(QStringLiteral("RibbonTabs"));
            tab_ok && tb != nullptr && tab >= 0 && tab < tb->count()) {
            tb->setCurrentIndex(tab);
            pump(400);
        }
    }
    ...
    std::printf("[ribbon_shot] kind=%d ribbon=%d line_btn=%d tip='%s' main=0x%lx => %s\n", kind,
                ...
                ok ? "PASS" : "FAIL");
    std::fflush(stdout);
    // The widget tree as rendered (popouts and overlays are children, so they are in it). Under
    // QT_QPA_PLATFORM=offscreen the window keeps the exact width asked for above, which a tiling
    // compositor may not allow; the GL viewport is blank there, but the ribbon is what this shows.
    if (!out_png.empty()) {
        grab().save(QString::fromStdString(out_png));
    }
    if (qEnvironmentVariableIsSet("MUSACAD_DYN_HOLD")) {
